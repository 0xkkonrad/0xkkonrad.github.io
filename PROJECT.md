...
- [x] **Substack feed is now build-time** (no client JS). Source order in `layouts/partials/substack-feed.html`: (1) `assets/substack.rss.xml` — deploy.yml tries to curl the RSS, but Substack's Cloudflare 403s ALL GitHub-Actions/datacenter IPs, so in practice (2) `assets/substack.json` — the old Apps Script proxy (Google IPs pass; **returns exactly 5 items, so 5 is the max prod can show** — tweak the Apps Script to raise it), (3) direct `resources.GetRemote` (works locally from residential IPs), (4) plain Substack link. Call the partial with a page context (8 items) or `(dict "limit" N)`. Freshness via daily cron in `deploy.yml`.
...
- [x] **Home: two boxes** — curated "Selected writing" from `data/selected_writing.yaml` (edit that file to change the picks) + "Latest on Substack" showing the 5 most recent posts (limit set in `layouts/partials/home_info.html`).
- [x] **Footer hidden** (`hideFooter = true` in hugo.toml — drops the © + "Powered by Hugo & PaperMod" line; PaperMod's scripts live outside the footer block and keep working).

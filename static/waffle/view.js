...
function budget(state, day, ui) {
  ...
  return `<div class="budget-list">${['focus', 'waffle'].map(mode => {
...
function planFields(plan, mode, minimum = 1, selected = '') {
  return `<div class="${mode === 'draft' ? 'plan-fields' : 'adjust-fields'}">${['focus', 'waffle'].map(kind => `<div class="plan-field ${selected === kind ? 'chosen-field' : ''}"><div><span>${kind}</span><small>${hours(plan[kind])}</small></div><div class="stepper"><button data-step="-1" data-field="${kind}" data-plan="${mode}" aria-label="Fewer ${kind} sessions" ${plan[kind] <= (kind === 'focus' ? 1 : 0) || plan.focus + plan.waffle <= minimum ? 'disabled' : ''}>${icon('minus')}</button><output aria-label="${kind} sessions">${plan[kind]}</output><button data-step="1" data-field="${kind}" data-plan="${mode}" aria-label="More ${kind} sessions" ${plan.focus + plan.waffle >= 24 ? 'disabled' : ''}>${icon('plus')}</button></div></div>`).join('')}</div>`;
}
...
function patch(day) {
  const counts = totals(day), total = counts.focus + counts.waffle;
  ...
  return `<svg class="day-mark" viewBox="0 0 40 44" aria-hidden="true">${Array.from({ length: total }, (_, index) => `<rect data-unit="${index < counts.focus ? 'focus' : 'waffle'}" x="${left + index % columns * (size + gap)}" y="${top + Math.floor(index / columns) * (size + gap)}" width="${size}" height="${size}" rx="1.4" fill="${index < counts.focus ? '#433321' : '#f3c345'}"/>`).join('')}</svg>`;
}
...
function sheet(state, ui) {
  ...
  if (type === 'options') {
    ...
    content = `<div class="locked-heading">${icon('check')}<span>${day.committedAt ? 'committed' : 'draft'} · ${hours(day.focus + day.waffle)}</span></div><div class="locked-row"><span>focus</span><strong>${day.focus} × 30m</strong></div><div class="locked-row"><span>waffle</span><strong>${day.waffle} × 30m</strong></div><div class="sheet-links">${day.committedAt && !day.off ? row('edit sessions', 'targets') : ''}${row('sessions', 'history')}${row('end day', 'end', !day.committedAt || !!day.endedAt || state.active?.phase === 'running' || state.active?.phase === 'review')}${!ui.standalone ? row('install waffle', 'install') : ''}${row('settings', 'settings')}${ui.updateReady ? row('update available', 'update') : ''}</div>`;
  }
  if (type === 'targets') {
    title = 'today’s plan';
    const draft = ui.sheet.draft, min = minimumTotal(state, date), active = state.active?.date === date ? state.active : null;
    content = `${planFields(draft, 'targets', min, ui.sheet.mode)}<p class="adjust-note">${day.sessions.length} completed${active ? active.phase === 'review' ? ' · 1 to rate' : ' · 1 in progress' : ''}</p>${primary(`save · ${hours(draft.focus + draft.waffle)}`, 'save-plan', '', draft.focus + draft.waffle < min)}`;
  }
  if (type === 'off') {
    title = 'day off?';
  ...
  if (type === 'end') {
    ...
    content = `${budget(state, day, ui)}<p class="sheet-note">${counts.focus >= day.focus && counts.waffle <= day.waffle ? 'focus target met' : 'focus target missed'}${partial ? `<br><span class="muted">${Math.floor(partial / 60000)}m unfinished · not counted</span>` : ''}</p>${primary('end day', 'confirm-end', 'check')}`;
  }
  if (type === 'correct') {
  ...
  if (type === 'install') {
    title = 'install waffle';
    content = `<p class="sheet-note">${ui.installReady ? 'Add Waffle to your home screen.' : 'Open your browser menu and choose “Add to Home screen” or “Install app”.'}</p>${ui.installReady ? primary('install', 'install-now') : ''}`;
  }
  if (type === 'restore') {
